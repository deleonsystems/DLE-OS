...
if (-not $ElevatedStage) {
    ...
    $childEvidence = Join-Path $artifactRoot (
        "RECEIVING_HISTORY_DEPLOYMENT_$stamp.json")
    $arguments = @(
    ...
    if (
        ...
        throw "Elevated Receiving History deployment failed: $childEvidence"
    }
...
try {
    foreach ($relative in @(
        ...
        $source = Join-Path (
            Join-Path $repository 'Tools\ReceivingHistory\ServerOverlay') `
            $relative
    ...
    $metadata = Invoke-RestMethod (
        ...
        'receiving-history/metadata') -TimeoutSec 10
    $sample = Invoke-RestMethod (
        'http://DLE-OS-HOST:5042/api/platform/live/v1/' +
        'receiving-history?page=1&pageSize=1') -TimeoutSec 10
    ...
    if (
        ...
        throw 'Receiving History post-publication HTTP qualification failed.'
    }
    ...
    $evidence.ReceivingHistoryMetadata = $metadata
...
if ($evidence.Verdict -ne 'PASS') {
    throw "Receiving History deployment failed: $($evidence.Error)"
}
