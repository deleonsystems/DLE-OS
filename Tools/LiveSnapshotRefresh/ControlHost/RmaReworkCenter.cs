...
internal sealed class RmaReworkService
{
    ...
    public async Task<object> CreateAsync(RmaReworkCreateRequest request, string authenticatedUser, CancellationToken token)
    {
        ...
        var interpretations = await ResolveAutomaticInterpretationsAsync(members, token);
        await repository.CreateAsync(caseId, caseType, customerRma, internalReference, notes,
            authenticatedUser, correlationId, currentReview.EvidenceToken, members, interpretations, token);
        ...
        string authenticatedUser, CancellationToken token)
    {
        ...
        var interpretation = (await ResolveAutomaticInterpretationsAsync([member], token))
            .GetValueOrDefault(member.Identity.Key);
        await repository.AddMemberAsync(caseId, member, reference, authenticatedUser,
            ...
            NormalizeOptional(request.Notes, 1000), interpretation, token);
        return await GetAsync(caseId, token);
    }
    ...
    private async Task<Dictionary<string, HistoricalWorkOrderEvidence>> ResolveAutomaticInterpretationsAsync(
        IReadOnlyList<RmaReworkMemberEvidence> members, CancellationToken token)
    {
        var result = new Dictionary<string, HistoricalWorkOrderEvidence>(StringComparer.Ordinal);
        foreach (var member in members.Where(item => item.QuantityOrdered < 0 &&
                     !string.IsNullOrWhiteSpace(item.RelatedWorkOrderNumber)))
        {
            var relationship = await operationalRelationships.LoadAsync(new LineKey(
                member.Identity.CustomerNumber, member.Identity.SalesOrderNumber, member.Identity.LineNumber), token);
            var historical = relationship.HistoricalWorkOrders.FirstOrDefault(item =>
                string.Equals(item.WorkOrderNumber, member.RelatedWorkOrderNumber, StringComparison.Ordinal));
            if (historical is not null) result[member.Identity.Key] = historical;
        }
        return result;
    }
...
internal sealed class RmaReworkRepository
{
    ...
    public async Task<Dictionary<string, RmaReworkMembership>> GetActiveMembershipsAsync(IReadOnlyList<RmaReworkLineIdentity> identities, CancellationToken token)
    {
        ...
        foreach (var identity in identities)
        {
            var command = new SqlCommand("SELECT TOP(1) CaseId,MemberSequence,COALESCE(NULLIF(CustomerRmaNumber,''),InternalReference) FROM operational.vw_ActiveRmaReworkCaseMember WHERE CustomerNumber=@C AND SalesOrderNumber=@S AND SalesOrderLineNumber=@L;", connection);
            ...
            if (await reader.ReadAsync(token)) result[identity.Key] = new(reader.GetGuid(0), reader.GetInt32(1), reader.GetString(2));
        }
        return result;
    }
...
SELECT TOP(1) CaseId,MemberSequence,COALESCE(NULLIF(CustomerRmaNumber,''),InternalReference)
...
""", connection, transaction);
        ...
        return await reader.ReadAsync(token)
            ? new RmaReworkMembership(reader.GetGuid(0), reader.GetInt32(1), reader.IsDBNull(2) ? "" : reader.GetString(2))
            : null;
    }
    ...
    public async Task CreateAsync(Guid caseId, string caseType, string? customerRma, string? internalReference,
        ...
        IReadOnlyList<RmaReworkMemberEvidence> members,
        IReadOnlyDictionary<string, HistoricalWorkOrderEvidence> interpretations,
...
""", connection, transaction);
            ...
            if (interpretations.TryGetValue(member.Identity.Key, out var interpretation))
                await InsertAutomaticInterpretationAsync(connection, transaction, caseId, index + 1,
                    member, interpretation, actor, token);
        }
...
""", connection, transaction);
    ...
    public async Task AddMemberAsync(Guid caseId, RmaReworkMemberEvidence member,
        ...
        string? note, HistoricalWorkOrderEvidence? interpretation, CancellationToken token)
    {
        await using var connection = new SqlConnection(connectionString); await connection.OpenAsync(token);
        await using var transaction = (SqlTransaction)await connection.BeginTransactionAsync(IsolationLevel.Serializable, token);
...
""", connection, transaction);
        ...
        if (interpretation is not null)
            await InsertAutomaticInterpretationAsync(connection, transaction, caseId, nextSequence,
                member, interpretation, actor, token);
...
""", connection, transaction);
    ...
    private static async Task InsertAutomaticInterpretationAsync(SqlConnection connection,
        SqlTransaction transaction, Guid caseId, int memberSequence, RmaReworkMemberEvidence member,
        HistoricalWorkOrderEvidence interpretation, string actor, CancellationToken token)
    {
        var current = new SqlCommand("""
SELECT TOP(1) EventId
FROM operational.SalesOrderLineWorkOrderInterpretationEvent WITH (UPDLOCK,HOLDLOCK)
WHERE CustomerNumber=@Customer AND SalesOrderNumber=@SalesOrder AND SalesOrderLineNumber=@Line
ORDER BY EventSequence DESC;
""", connection, transaction);
        current.Parameters.AddWithValue("@Customer", member.Identity.CustomerNumber);
        current.Parameters.AddWithValue("@SalesOrder", member.Identity.SalesOrderNumber);
        current.Parameters.AddWithValue("@Line", member.Identity.LineNumber);
        var currentValue = await current.ExecuteScalarAsync(token);
        var priorEventId = currentValue is Guid value ? value : (Guid?)null;
...
VALUES(@EventId,@CaseId,@MemberSequence,@Customer,@SalesOrder,@Line,NULL,@Historical,@Role,
 'RMA_DECISION_PENDING',@Reason,@Actor,@Supersedes,NEWID());
""", connection, transaction);
        insert.Parameters.AddWithValue("@EventId", Guid.NewGuid());
        insert.Parameters.AddWithValue("@CaseId", caseId);
        insert.Parameters.AddWithValue("@MemberSequence", memberSequence);
        insert.Parameters.AddWithValue("@Customer", member.Identity.CustomerNumber);
        insert.Parameters.AddWithValue("@SalesOrder", member.Identity.SalesOrderNumber);
        insert.Parameters.AddWithValue("@Line", member.Identity.LineNumber);
        insert.Parameters.AddWithValue("@Historical", interpretation.WorkOrderNumber);
        insert.Parameters.AddWithValue("@Role", interpretation.RelationshipRole);
        insert.Parameters.AddWithValue("@Reason",
            "RMA/Rework membership issued; canonical Work Order retained as historical evidence only.");
        insert.Parameters.AddWithValue("@Actor", actor);
        insert.Parameters.AddWithValue("@Supersedes", (object?)priorEventId ?? DBNull.Value);
...
internal sealed record RmaReworkMembership(Guid CaseId,int MemberSequence,string CaseReference)
{
    public RmaReworkMembership(Guid caseId, string caseReference) : this(caseId, 0, caseReference) { }
}
