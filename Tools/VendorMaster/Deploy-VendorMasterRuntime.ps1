...
if (-not $ElevatedStage) {
    ...
    $childEvidence = Join-Path $artifactRoot (
        "VENDOR_MASTER_DEPLOYMENT_$stamp.json")
    $arguments = @(
    ...
    if ($child.ExitCode -ne 0 -or
        ...
        throw (
            'Elevated Vendor Master deployment failed. Evidence: ' +
            $childEvidence)
    }
...
try {
    foreach ($relative in @(
        ...
        $source = Join-Path (
            Join-Path $repository 'Tools\VendorMaster\ServerOverlay') $relative
    ...
    $metadata = Invoke-RestMethod (
        'http://DLE-OS-HOST:5042/api/platform/live/v1/vendor-master/metadata') `
        -TimeoutSec 10
    $sample = Invoke-RestMethod (
        'http://DLE-OS-HOST:5042/api/platform/live/v1/vendor-master' +
        '?page=1&pageSize=50&vendorNumber=34') -TimeoutSec 10
    if ($readiness.readinessVerdict -ne 'Ready' -or
        $metadata.vendorCount -ne 805 -or
        $metadata.vendorAddressCount -ne 106 -or
        $sample.totalItems -ne 1 -or
        $sample.items[0].vendorNumber -ne '000034' -or
        $sample.items[0].vendorName -ne 'WALKER COMPONENT GROUP') {
        throw 'Vendor Master post-publication HTTP qualification failed.'
    }
    ...
    $evidence.VendorMasterMetadata = $metadata
    $evidence.KnownVendor = $sample.items[0]
...
if ($evidence.Verdict -ne 'PASS') {
    throw "Vendor Master deployment failed: $($evidence.Error)"
}
