...
$liveIdentity = 'DLE-OS-HOST\DLE-OS-LIVE-API'
...
if (-not $ElevatedStage) {
    ...
    $childEvidence = Join-Path $artifactRoot (
        "SUPPORTING_CODE_TABLES_DEPLOYMENT_$stamp.json")
    $arguments = @(
    ...
    if (-not $child.HasExited) {
        throw (
            'Elevated deployment child did not exit within five minutes. ' +
    ...
    if (
        ...
        throw (
            'Elevated Supporting Code Tables deployment failed. Evidence: ' +
            $childEvidence)
    }
...
try {
    foreach ($relative in @(
        ...
        $source = Join-Path (
            Join-Path $repository 'Tools\SupportingCodeTables\ServerOverlay') `
            $relative
    ...
    if ($null -eq $evidence.PromotionBrokerProcessId) {
        ...
        New-Item -ItemType Directory -Path $promotionLogRoot -Force | Out-Null
        Start-Process `
    ...
    $metadata = Invoke-RestMethod (
        ...
        'reference-codes/metadata') -TimeoutSec 10
    $sample = Invoke-RestMethod (
        'http://DLE-OS-HOST:5042/api/platform/live/v1/' +
        'reference-codes?page=1&pageSize=50&codeDomain=Purchasing' +
        '&codeType=PaymentTerms&codeValue=01') -TimeoutSec 10
    ...
    if (
        ...
        $metadata.referenceCodeCount -ne 1209 -or
        $sample.totalItems -ne 1 -or
        $sample.items[0].codeValue -cne '01' -or
        [string]::IsNullOrWhiteSpace($frontendBuild.frontendBuildId)
    ) {
        throw 'Supporting Code Tables post-publication HTTP qualification failed.'
    }
    ...
    if (
        $evidence.LiveRuntimeOwner -ine $liveIdentity -or
        $evidence.RefreshControlOwner -ine $operator -or
        $evidence.PromotionBrokerOwner -ine $operator
    ) {
        throw 'One or more runtime identities are outside the approved boundary.'
    }
    $evidence.Readiness = $readiness
    $evidence.ReferenceCodeMetadata = $metadata
...
if ($evidence.Verdict -ne 'PASS') {
    throw "Supporting Code Tables deployment failed: $($evidence.Error)"
}
