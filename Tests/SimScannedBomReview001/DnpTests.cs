...
internal static class DnpTests
{
    internal static async Task Run(string repository,JsonSerializerOptions options)
    {
        ...
        var node=JsonNode.Parse(original)!["records"]!.AsArray().Single(n=>(string?)n!["intakeId"]=="RFQI-SIM-0041")!.DeepClone();
        ...
        // Requested examples are fixtures only: NOT_USED source positions are excluded by the unchanged builder.
        foreach(var pair in new[]{("6","Q2"),("7","Q3")}){
            var values=new Dictionary<string,string>{{"lineNumber",pair.Item1},{"partNumber",""},{"quantity","0"},{"designators",pair.Item2},{"description","NOT USED"}};
            rows.Add(new(rows.Count,new(values),new(values),[],SimCandidateBomProvider.Fields.ToDictionary(k=>k,_=>"GOVERNING"),false,null,null,[],RowId:"dnp-fixture-"+pair.Item1,ManufacturerIdentity:new([],"No identity",[])));
        }
        ...
        for(int index=initial.Rows.Length;index<rows.Count;index++){
        ...
        for(int i=0;i<rows.Count;i++){var b=await Bom();var r=b.Rows[i];if(r.ReviewState.Reviewed)continue;var v=Values(r);if(!int.TryParse(v["lineNumber"],out var n)||n<1)v["lineNumber"]=(i+1).ToString();if(!decimal.TryParse(v["quantity"],out var q)||q<=0)v["quantity"]="1";var manual=r.IdentityBasis=="CUSTOMER_PN"||r.ManufacturerIdentity?.Proposals.Any(p=>!string.IsNullOrWhiteSpace(p.PartNumber))==true?null:"TEST-PART-"+i;await store.CandidateBomAsync(record.IntakeId,persona,new(b.Id,i,v,WorksheetAcceptance:new(r.ReviewState.Token,r.ComponentType,r.AssemblyIdentity?.PartNumber??"TEST-ASSEMBLY",ManualPartNumber:manual)));}
        ...
        var ready=await Bom();await store.CompleteBomReview(record.IntakeId,new(ready),persona);var accepted=(await Read()).TechnicalReview!.BomAcceptances!.Last().Candidate;
        Check(accepted.Rows.TakeLast(2).All(r=>r.ComponentType=="DNP"&&r.Values["description"]=="DO NOT POPULATE"&&r.Values["designators"].StartsWith("Q")),"Accepted BOM preserves both DNP instructions and Ref Des");
        await File.WriteAllTextAsync(Path.Combine(root,"accepted.json"),JsonSerializer.Serialize(await Read(),options));
        Check(original.SequenceEqual(await File.ReadAllBytesAsync(Path.Combine(repository,".sim-state/data/rfq-intakes.json"))),"normal SIM data and Candidate inclusion unchanged");
