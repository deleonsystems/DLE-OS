...
assert.match(markup, />How to Sync</);
assert.match(markup, /DLE-OS-HOST\\DLE-OS/);
assert.match(markup, /C:\\DLE-OS\\Repositories\\DLE-OS\\Tools\\InvoiceHistory\\Start-InvoiceHistoryRefresh\.cmd/);
assert.match(source, /Automatic sync temporarily unavailable/);
...
assert.equal(elements.get("invoiceHistorySyncStatus").textContent,"Automatic sync temporarily unavailable");
assert.match(elements.get("invoiceHistoryLastAttempt").textContent,/unavailable while the sync service is offline/);
