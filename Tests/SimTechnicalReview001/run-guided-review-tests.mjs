...
assert.match(node('technicalReviewDetail').innerHTML, /Technical Package Review/);
assert.match(node('technicalReviewDetail').innerHTML, /2 need review/);
await click('identity-change');
handlers.change({target:{dataset:{identityType:''},value:'BOM_ONLY'}});
await click('identity-confirm');
handlers.change({target:{dataset:{pnField:'basis'},value:'MANUFACTURER'}});
...
assert.match(node('technicalReviewDetail').innerHTML, /Candidate BOM/);
assert.match(node('technicalReviewDetail').innerHTML, /EXTRACTED-PART/);
assert.match(node('technicalReviewDetail').innerHTML, /<table class="candidate-table candidate-workbench-table"/);
assert.match(node('technicalReviewDetail').innerHTML, /1 unresolved/);
...
assert.match(node('technicalReviewDetail').innerHTML, /Source \/ Evidence/);
...
assert.match(node('technicalReviewDetail').innerHTML, /data-status="Manually Corrected"/);
await click('materials');
await workspace.render();
