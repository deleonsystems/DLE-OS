...
const expectComplete = () => {
  assert.match(html(), /Continue Technical Review/);
  assert.match(html(), /data-technical-review-action="manufacturing"/);
  assert.match(html(), /Back to Queue/);
  assert.doesNotMatch(html(), /technical-review-phase|No Longer Required|Add Technical Document/);
};
...
assert.doesNotMatch(html(), /technical-review-phases/);
...
await click('review-back');
assert.match(html(), /Assembly Type<\/dt><dd>PCB Assembly/);
assert.equal((html().match(/>Continue Technical Review<\/button>/g) || []).length, 1);
await workspace.render();
await workspace.openReview(record.intakeId);
await click('start');
assert.match(html(), /Have we built this assembly before/);
