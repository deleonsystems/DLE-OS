...
function IsRuntimePrincipal([string]$UserId){$UserId-ieq$runtimeIdentity-or$UserId-ieq'DLE-OS-DEV-CONTROL'}
...
try{
    ...
    if(-not$candidate.Settings.Enabled-or$candidate.State-ne'Running'-or-not(IsRuntimePrincipal ([string]$candidate.Principal.UserId))-or[string]$candidate.Principal.LogonType-ne'Password'-or[string]$candidate.Settings.ExecutionTimeLimit-ne'PT0S'-or[string]$candidate.Settings.MultipleInstances-ne'IgnoreNew'-or[string]$oldAction.WorkingDirectory-ne$oldRelease){throw 'Candidate task is not at the recovered qualified baseline.'}
    ...
    $after=Get-ScheduledTask -TaskPath $candidatePath -TaskName $candidateName;if(-not(IsRuntimePrincipal ([string]$after.Principal.UserId))-or[string]$after.Principal.LogonType-ne'Password'-or[int]$after.Settings.RestartCount-ne4-or[string]$after.Settings.RestartInterval-ne'PT2M'-or[string]$after.Settings.ExecutionTimeLimit-ne'PT0S'-or[string]$after.Settings.MultipleInstances-ne'IgnoreNew'){throw 'Re-registered task does not match the approved principal/settings.'}
