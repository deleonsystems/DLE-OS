(function registerKittingReadModel(window) {
  ...
  function buildReadModel(options = {}) {
    ...
    lines.forEach(line => {
      const authority = resolveLineAuthority(line, approvalsByLineKey.get(line.lineKey));
      const rmaMembership = rmaReworkByLineKey.get(line.lineKey);
      if (rmaMembership) {
        const caseKey = cleanText(rmaMembership.caseId || rmaMembership.caseReference || line.lineKey);
        const group = rmaGroups.get(caseKey) || {
          caseId: cleanText(rmaMembership.caseId),
          caseReference: cleanText(rmaMembership.caseReference) || "Active RMA/Rework Case",
          caseType: cleanText(rmaMembership.caseType) || "RMA / Rework",
          caseStatus: cleanText(rmaMembership.caseStatus) || "ACTIVE",
          customerNumber: line.customerNumber,
          customerName: line.customerName,
          lines: []
        };
        group.lines.push({ ...line, authority, rmaReworkCase: rmaMembership });
    ...
    const counts = buildCounts(lines, queues, rmaReworkByLineKey);
  ...
  function buildRmaReworkRow(group) {
    ...
    return Object.freeze({
      ...
      nextRequiredAction: "Review RMA/Rework disposition"
    });
  }

  function buildCounts(lines, queues, rmaReworkByLineKey) {
    const primaryRows = [
    ...
    return Object.freeze({
      ...
      rmaReworkExcludedLines: lines.filter(line => rmaReworkByLineKey.has(line.lineKey)).length,
